...
- **測試呼叫得到內層函式，不代表外層有人啟動它。** 2026-09-22：`sweeper()` 的
  迴圈寫好了、`sweep()` 也有八個測試全綠，但 `main.py` 連 `import authorize`
  都沒有 —— 那條防禦在正式環境**一次都沒跑過**。

  「它會不會被執行」要驗 **wiring**，不是驗單元：跑一次真的 lifespan、去
  `asyncio.all_tasks()` 裡找那個 task，關機後再確認它不見了（`6905aec`）。
  同理適用於任何「寫好但要有人叫它」的東西：背景任務、signal handler、
  middleware、router 的 `include_router`。
...
做成我來代跑頁底部的區塊」）—— 使用者選了獨立分頁，MinIO 連結搬到這裡來。

#### admin 怎麼判定

`hub/.env` 的 **`ADMIN_EMAILS`**（逗號分隔）。**六個 email 已經設好了**，
程式碼一落地就生效；`.env.example` 要補上這一行與說明。

- 不加資料庫欄位、不做管理介面 —— 名單改動的頻率是「幾個月一次」，改 env 重啟就好
- **沒設就是所有人都不是 admin**，不要寫成「沒設就都是」
- 比對要**小寫化 + trim** 再比，人手維護的清單一定會有大小寫與空白

#### 導覽列放得下，我算過了

容器 900px，把中文低估補回之後真實內容約 **777.6px**，餘裕 **122px**。
第七個分頁「管理」約需 64px（2 個中文字 28 + padding 28 + gap 8），
加完約 841.6px、**還剩 58px**。而且那個分頁只對 admin 渲染，其他人畫面不變。

#### 這頁放什麼：**看得到什麼壞了，不能瀏覽誰欠誰**

這個分界是使用者定的。`_get_job` 刻意限制「誰能看到誰的 job」，而在一個以人情債
為核心的產品裡，「誰用得兇、誰欠誰多少」是**社交敏感資訊** —— 那不是技術問題，
是這個工具會不會被同事信任的問題。

| 放 | 不放 |
|---|---|
| 系統健康（各元件活著沒） | 全站 job 瀏覽 |
| 聚合數字（幾個人、幾個 job、總花費、成功率），**不指名** | 誰欠誰、誰用得兇 |
| **卡住的 job** 清單 —— 只列異常的，含 job id、狀態、卡多久、哪台 worker | 任何 job 內容（紅線 1） |

「卡住」的判定要寫死在程式碼裡並註明理由，不要每個人自己抓一個數字：
`queued` 超過 10 分鐘沒被領走、或 `running` 超過該 worker 的逾時設定 + 2 分鐘寬限。

#### 🚨 每一個綠燈都要是真的量到的

這是這一格最重要的一條。**量不到的東西就不要放燈** —— 一個永遠綠的健康指示器
比沒有指示器危險，因為它會被相信。（今天已經抓過同一個形狀七次。）

- **worker 線上**：`last_seen_at` 有現成的，會動
- **MinIO**：打 `:9000/minio/health/live`（實測回 200）
- **DB migration 版本**：`main.py` 已經在啟動時檢查，把版本讀出來即可
- **egress proxy**：那是 worker 端的 docker 容器，**hub 很可能根本檢查不到**。
  檢查不到就**不要放這一項**，或明寫「這項無法從 hub 確認」。不要放一顆假燈
- **hub 版本**：如果專案沒有版本概念，就別發明一個 —— 放 git sha 或直接不放

#### 端點

新開 `hub/app/routers/admin.py`，全部路徑 `/api/admin/*`，**每一個都要擋非 admin
（403）** —— 不是靠前端不顯示。另外 `/me` 多一個 `is_admin: bool`。

#### ⚠️ 兩個檔案要跟 A 對一下

`hub/app/config.py` 與 `hub/app/routers/auth.py` —— A 正在做授權那條路，
可能同時在動。動之前 `git status`，或直接問它。

#### 交付

**量得到的與量不到的分三區**（`checks` / `facts` / `unknown`），不是一排燈：

| 項目 | 怎麼處理 | 為什麼 |
|---|---|---|
| 資料庫 | 🟢/🔴 真的跑一個 query | |
| MinIO | 🟢/🔴 打 `/minio/health/live` | 把位址指到死埠會變紅，驗過 |
| worker | 🟢/🔴 `last_seen_at` | |
| schema 版本 | **沒有燈**，只顯示版本字串 | hub 啟動時就檢查、不符會拒絕啟動，做成燈的話**永遠是綠的** |
| egress proxy | 放進「無法從這裡確認」並寫原因 | 它是代跑者機器上的容器，hub 連不到也沒有回報管道 |

「卡住」的門檻：排隊 600 秒、執行中 720 秒。⚠️ **後者是假設不是量到的** ——
`WorkerConfig` 沒有回報 worker 自己的逾時，代跑者調長的話他的 job 會提早被列進來。
要精確就得讓 worker 回報那個值（不在這次範圍）。

空清單與「偵測不到」在畫面上長得一樣，所以測試裡真的塞一筆逾時的 job 進資料庫
（交易包住、結束 rollback）確認抓得到，並確認 prompt 一個字都沒跟著出來。

⚠️ **第七個分頁讓導覽列在 900px 視窗從一行變兩行**（補回中文寬度後內容 841.5px，
與派工估的 841.6 一致）。1024 以上仍是一行，而且只影響管理者。

#### MinIO 連結一起搬過來

連同它現有的文案（整站共用 bucket、沒有「只看自己」的權限、你會看到別人的
對話與檔案）。**`s3_console_url` 改成只回傳給 admin** —— 現在它對所有人都出現在
`/me` 裡，前端不渲染不等於沒送出去。

**「我來代跑」那份現在就刪掉了，沒有等舊路徑退場**：那段文案自己寫著
「這頁之外的地方不會出現這個連結，原因就是這個」—— 留兩份的話那句就是假的，
而它正是在解釋一個安全限制。

---

### ~~→ 誰有空都可以：授權的背景清潔工從來沒被啟動（2026-09-22，**bug**）~~ ✅ 已完成（`6905aec`）

`hub/app/authorize.py` 的 `sweeper()` 寫好了、註解寫清楚了為什麼需要它、
`sweep()` 也有測試 —— **但它沒有被接上**。查證：

- `sweeper` 唯一的呼叫者是它自己（第 233 行，迴圈內）
- 測試呼叫的是同步的 `sweep()`，不是那個迴圈
- **`main.py` 根本沒有 import `authorize`**，`lifespan` 裡只有 migration 檢查、
  金鑰檢查、`ensure_bucket`

所以 8 個生命週期測試全綠，而那條防禦在正式環境一次都沒有跑過。

**後果**（照它自己註解寫的）：任何人開了授權頁又關掉，就留下一個永不結束的
`claude setup-token` 子行程。損害有兩道天然上限 —— `PR_SET_PDEATHSIG`
讓它隨 Hub 一起死、「一人一個」讓每位代跑者最多累積一個 —— 所以不是災難，
**但它正是那段程式碼宣稱要防止的事**。

**修法**：`lifespan` 裡 `asyncio.create_task(sweeper())`，`yield` 之後 cancel 掉。

**驗收不要只看測試綠**：那正是它漏掉的方式。要能證明**跑起來的 app 裡真的有
那個 task** —— 例如啟動後檢查 `asyncio.all_tasks()`，或讓 sweeper 第一圈印一行。

> 這是今天第九次同一個形狀：**寫了、測了、記錄了，但沒有接上**。
> 前八次見〈第二條規則〉。這次的教訓要補進去：**測試呼叫內層函式，不代表
> 外層迴圈有人啟動它。** 驗「它會不會被執行」要看 wiring，不是看單元測試。
