...
**網路**：~~預設只放行 `api.anthropic.com` + MinIO~~ **2026-09-23 起預設開外網，
代跑者可自己關**（關了就是原本的白名單模式）。

> 為什麼翻過來：這段寫的時候 job 在出租者自己的電腦上跑，擋外網保護的是那台電腦。
> §1 改成共用主機之後，容器裡沒有任何人的檔案、跑完銷毀，保護對象不存在了；代價
> 卻每天在付 —— 第一個正式站的 job 就是貼一個 GitHub 連結、被擋、US$0.10 換來一句
> 「請把內容貼給我」。還在的風險只剩容器裡那張 8 小時的 access token 可能被惡意
> 內容送出去：有上限、可撤銷，所以交給代跑者自己決定。完整約束見 security.md 紅線 3。
...
> 2026-09-23：最底那格照舊不掛債，但帳本會把「不到一杯的往來」按人加總顯示
> （幾次、合計），不然債主分不出「沒人來借」跟「來借了六次、每次都不到一杯」。
> 不是新規則，是脈絡；見 web-spec §6。
...
#### ✅ refresh 會滾動，而且期限跟著往後推（2026-09-23 實測）

改用「讓 CLI 自己去 refresh」的測法，一次就成：把憑證複製到臨時 HOME、把
`expiresAt` 改成過去的時間、跑一次 `claude -p pong`，然後比對那個複本。

- **refresh token 換了新的一張**（指紋不同）—— 是滾動式的
- **而且期限往後推**：同一份憑證在正式位置上自己 refresh 之後，
  `refreshTokenExpiresAt` 從 `10-19 21:45` 變成 `10-20 20:01`，也就是
  **從「現在」重新算約 27 天**，不是固定從第一次登入起算

**所以「每 30 天要重新登入」那條不成立** —— 只要 Hub 持續在 refresh，期限就一直
往後滾。這是 #13 原本最可能致命的那一條，現在解除了。

一個順帶的觀察：測試輪替掉 refresh token 的兩分鐘後，**正式位置那份用它自己的
（舊的）refresh token 仍然 refresh 成功**。所以輪替不會立刻作廢前一張 ——
這對 Hub 有實際意義：兩個地方同時持有同一個帳號的憑證時，不會一 refresh 就互相
踢掉。**但這是觀察到的行為，不是承諾**，不要拿它當設計前提。

#### ⚠️ 手刻協定 vs 讓 CLI 做，差別是決定性的

同一件事，自己打 `POST /v1/oauth/token` 是 403 然後一路 429；讓 CLI 去做，
一次就過。**這條寫下來不是為了記錄失敗，是為了定調實作方式**：Hub 不要自己
實作 OAuth，它已經在用 pty 驅動 CLI 了（#9），refresh 也走同一條路。
