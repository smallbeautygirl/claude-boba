...
| job 預設能連外網，違反紅線 3「egress 白名單」 | 紅線 3 在 2026-09-23 **改寫過**：擋外網保護的是「出租者自己的電腦」，共用主機之後那台機器不存在了。預設開、代跑者可關，關了就是原本的白名單。不要把預設改回關 —— 那是替每個貼連結的人各付一次 US$0.10 的學費。見 SPEC §4.4 與 security.md 紅線 3 |
...
2. ~~隱私勾選的對象變了，文案還沒改。~~ **2026-09-23 已改**：提交頁現在點名的是
   站台管理者（名字由 hub 的 `admin_names` 帶下來），並明講代跑者看不到。
   見 web-spec〈隱私勾選〉。
