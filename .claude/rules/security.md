...
### 3. 網路模式是代跑者的決定，不是主機的設定（2026-09-23 改寫）

> **這條原本寫「Egress 白名單是安全邊界，不是效能設定」，預設只放行
> `api.anthropic.com` 與 MinIO。2026-09-23 改成預設開外網、代跑者可關。**
> 這是知情的取捨，不是有人沒讀到規則。

原本的理由：借用者提交的內容會在**出租者的機器上**被 Claude 執行，容器能出網
等於借用者能把那台機器上的東西送出去、或把它當跳板。**那台機器已經不存在了** ——
2026-09-22 起 job 在共用主機的拋棄式容器裡跑（§1），容器裡沒有任何人的檔案，
跑完就銷毀。擋外網保護的對象消失了，代價卻每天在付：每個貼連結進來的人都撞一次
「打不開」，第一個正式站的 job 就是（US$0.10 換來一句「請把內容貼給我」）。

**還在的風險只有一個，要講清楚：** 容器裡有代跑者那張 8 小時的 access token
（紅線 2 第三列）。開了外網，一段惡意的貼上內容就能叫 Claude 把它送去任何地方。
損失有上限（8 小時、可撤銷、只有 inference 額度），但不是零 —— 所以這是**代跑者
的決定**，不是站台替他做的。

- 預設**開**。`LendingSetting.allow_full_network` 的 ORM default 與 DB server_default
  都是 true，兩份要一致（`tests/test_network_default_open.py`）
- 代跑者可以在「我來代跑」自己關掉。關掉的人拿到的是原本的白名單模式：只放行
  `api.anthropic.com` 與 MinIO（`worker/egress/allowlist`）
- 提交頁要標示所選代跑者的網路模式，而且只在他選到**白名單**的人時警告
  「連結打不開」—— 預設已經是開的，對每個人都警告等於沒警告
- 開關的文案要講的是**上面那段真實風險**（token 可能被送出去），不是「可能有風險」
  這種空話，也不要反過來把關掉講成偏執
- 不要因為預設開了就把 egress proxy 拆掉：關著的人還在用它，而且它是「關」
  這個選項唯一的實作
