...
| 2026-09-24 | 排行榜：欠債王、金主榜、本月最大宗（web-spec §7）。最後一個未實作的功能 |

還沒做、或做了還沒驗：

- ~~OAuth 授權還沒端到端走通過一次~~ 2026-09-23 傍晚在正式站走通了：授權、
  派單、跑完、記帳一路到底。當天上午的「限流」是 hub 送的 User-Agent 抄錯被
  Cloudflare 一律回 429（SPEC §11 #13 的更正）。
- ~~隱私勾選的文案還點名代跑者~~ 2026-09-23 已改成點名站台管理者
  （web-spec〈隱私勾選〉）。[CLAUDE.md](CLAUDE.md) 待辦 1（交出 token 那一步的
  揭露文案）還在。
- **LICENSE**：見「授權」。
